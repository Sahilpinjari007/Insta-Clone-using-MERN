...
import { getUserTaggedPosts } from '../../../../action/post';
...
const NavPostPageCard = ({ cardData }) => {
  ...
  return (
    <div className="nav-tagged-post-page-card" onClick={handlePostOnClick}>
...
const NavTagsPage = () => {
  ...
  const [userTaggedPosts, setUserTaggedPosts] = useState([]);

  const getUserAllTaggedPosts = async () => {
    const newArr = [];
    const response = await getUserTaggedPosts(profileUser.userId);

    while (response.result.length > 0) {
      newArr.push(response.result.splice(0, 3));
    }

    setUserTaggedPosts(newArr)
  };

  useEffect(() => {
    getUserAllTaggedPosts();
  }, [profileUser]);

  return (
    <div className="nav-tagged-post-page">
      {userTaggedPosts?.map((elem, i) => {
        return <div key={i} className="nav-tagged-post-page-row">{elem?.map((item, i) => <NavPostPageCard cardData={item} key={i}/>)}</div>;
      })}
    </div>
  );
}
